...
const modelImagesByNameKey: Record<string, string[]> = {
  'taro gp 2': ['/bikes/Taro_-TARO_GP_2-200cc.png'],
  ...
  'lifan v16s': ['/bikes/Lifan_-lifan_v16s_-250cc-1.png'],
...
function CatalogueInner() {
  ...
  const initialCategory = tab === 'others' ? 'OTHERS' : 'ORIGINAL';
            ...
            <div className="inline-flex rounded-full border border-secondary/40 bg-white/80 backdrop-blur-sm p-1 text-xs uppercase tracking-widest">
              <button
                onClick={() => {
                  ...
                  router.replace('/catalogue?tab=original');
                }}
              ...
              <button
                onClick={() => {
                  ...
                  router.replace('/catalogue?tab=others');
                }}
                className={`px-4 py-2 rounded-full transition-colors ${
                            ...
                            <div className="flex items-center">
                              ...
                              <div className="flex-1 min-w-0 px-6 py-6 flex flex-col justify-center">
                                ...
                                <div className="mt-3 flex flex-wrap gap-2">
                                  {bike.ccs.map((cc) => (
                                    <span key={cc} className="inline-flex items-center rounded-full border border-secondary/40 px-3 py-1 uppercase tracking-widest text-xs text-muted hover:border-primary hover:text-primary transition-colors">
                                      {cc} CC
                                    </span>
                                  ))}
                                </div>
...
export default function Catalogue() {
  return (
    <Suspense fallback={<div className="pt-32 pb-24 min-h-screen" />}>
      <CatalogueInner />
    </Suspense>
  );
}
