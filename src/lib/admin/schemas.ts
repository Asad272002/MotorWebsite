...
export const simpleBikeStockSchema = z.object({
  brandId: z.preprocess((value) => value === "" || value === "__new__" ? undefined : value, uuid.optional()),
  newBrandName: z.string().trim().max(100).optional().or(z.literal("")),
  ...
  if (!value.brandId && !String(value.newBrandName ?? "").trim()) {
    ctx.addIssue({ code: "custom", path: ["brandId"], message: "Select an existing brand or enter a new brand name." });
  }
});
