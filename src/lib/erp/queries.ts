...
export async function listStaffProfiles(): Promise<readonly StaffProfile[]> {
  ...
  const purge = await sb
    .from("profiles")
    .update({ created_password: null })
    .not("created_password", "is", null);
  if (purge.error) {
    console.error("[OW Motors stored password purge failed]", { code: purge.error.code });
  }
  let query = sb
    .from("profiles")
...
export const listStockMotorcycleModels = cache(async (): Promise<readonly {
  id: string;
  name: string;
  slug: string;
  ...
  const { data } = await sb
    .from("motorcycles")
    .select("id, name, slug, brand:brands(id, name, slug, is_active), variants:motorcycle_variants(id, is_active, is_default, cc, price, color_name)")
    .order("name", { ascending: true });
  type Row = {
    id: string;
    name: string;
    slug: string;
    brand?: { id: string; name: string; slug: string; is_active?: boolean | null } | null;
    variants?: {
      id: string;
  ...
  return ((data ?? []) as unknown as Row[])
    ...
    .map((row) => ({
      ...(() => {
        const activeVariants = (row.variants ?? []).filter((variant) => variant.is_active !== false);
        const sortedVariants = [...activeVariants].sort((a, b) => Number(b.is_default) - Number(a.is_default) || Number(a.cc ?? 0) - Number(b.cc ?? 0));
        const template = sortedVariants[0];
        return {
          template_variant: template ? { cc: Number(template.cc ?? 0), price: Number(template.price ?? 0) } : null,
          existing_colors: Array.from(
            new Set(
              activeVariants
                .map((variant) => String(variant.color_name ?? "").trim())
                .filter(Boolean),
            ),
          ),
        };
