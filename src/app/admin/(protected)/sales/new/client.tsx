...
export default function NewSalePageClient(props: {
  variants: Variant[]; stockUnits: StockUnit[]; banks: Bank[]; customers: Customer[]; myProfileId: string | null; canApplyDiscount: boolean;
}) {
  const { variants, stockUnits, banks, customers, canApplyDiscount } = props;
  ...
  const grossAmount = (chosen?.price ?? 0) * saleQuantity;
  const discountAmount = discountOpen && canApplyDiscount
    ? Number(discountValue.replace(/[^0-9]/g, "")) || 0
    : 0;
  const discountInvalid = discountAmount >= grossAmount && discountAmount > 0;
  ...
  const paymentStepComplete = paymentRowsComplete && !discountInvalid && totalAmount > 0 && paidAmount === totalAmount && payments.some(p => (Number(p.amount.replace(/[^0-9]/g, "")) || 0) > 0);
  ...
  const canSubmit = saleInfoStepComplete && paymentsValid && !discountInvalid && paidAmount === totalAmount && totalAmount > 0;
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
            ...
            <div>
              ...
              <div className={`${adminInputClass} flex items-center justify-between font-display text-xl font-bold text-[#C62828]`}>{pkr(totalAmount)}<span className="text-xs font-normal text-[#6B7280]">{chosen ? discountAmount > 0 ? `After ${pkr(discountAmount)} discount` : `${chosen.price?.toLocaleString("en-PK")} x ${saleQuantity}` : "Pick a bike"}</span></div>
      ...
      >
        <div className="space-y-3">
          {canApplyDiscount ? (
            <div>
              {!discountOpen ? (
                <button type="button" onClick={() => setDiscountOpen(true)} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-dashed border-[#C62828]/50 px-4 text-sm font-semibold text-[#C62828] hover:bg-[#FEF2F2]">
                  <Plus aria-hidden="true" className="h-4 w-4" />Add discount
                </button>
              ) : (
                <div className="grid grid-cols-1 gap-3 rounded-md border border-[#E5E7EB] bg-[#FAFAFA] p-4 sm:grid-cols-[1fr_auto] sm:items-end">
                  <div>
                    <label htmlFor="newSale-discountAmount" className={adminLabelClass}>Discount, PKR</label>
                    <input id="newSale-discountAmount" data-error-path="discountAmount" inputMode="numeric" value={discountValue} onChange={(event) => setDiscountValue(event.target.value.replace(/[^0-9,]/g, ""))} className={`${adminInputClass} ${discountInvalid || hasErr("discountAmount") ? ERROR_INPUT_RING : ""}`} placeholder="0" />
                    {discountInvalid ? <p className="mt-1 text-xs font-semibold text-[#C62828]">Discount must be less than the original price.</p> : hasErr("discountAmount") ? <p className="mt-1 text-xs font-semibold text-[#C62828]">{errText("discountAmount")}</p> : null}
                  </div>
                  <button type="button" onClick={() => { setDiscountOpen(false); setDiscountValue(""); }} className="inline-flex min-h-11 items-center justify-center rounded-md border border-[#D1D5DB] bg-white px-4 text-sm font-semibold text-[#374151] hover:bg-[#F3F4F6]">Remove</button>
                  {discountAmount > 0 && !discountInvalid ? (
                    <dl className="grid grid-cols-3 gap-3 border-t border-[#E5E7EB] pt-3 text-xs sm:col-span-2">
                      <div><dt className="text-[#6B7280]">Original</dt><dd className="mt-1 font-semibold text-[#111111]">{pkr(grossAmount)}</dd></div>
                      <div><dt className="text-[#6B7280]">Discount</dt><dd className="mt-1 font-semibold text-[#15803D]">- {pkr(discountAmount)}</dd></div>
                      <div><dt className="text-[#6B7280]">Total to pay</dt><dd className="mt-1 font-bold text-[#C62828]">{pkr(totalAmount)}</dd></div>
                    </dl>
                  ) : null}
                </div>
              )}
