...
import { listPartSales, listReceipts, listSales } from "@/lib/erp/queries";
...
export default async function SalesListPage() {
  const [sales, partSales, receipts] = await Promise.all([listSales(), listPartSales(), listReceipts()]);
  const receiptIdsBySaleId = Object.fromEntries(
    receipts
      .filter((receipt) => receipt.sale_id)
      .map((receipt) => [receipt.sale_id, receipt.id]),
  );
  const totals = {
  ...
  return (
    <div className="space-y-8">
      ...
      <SalesRegisterClient
        ...
        partSales={JSON.parse(JSON.stringify(partSales))}
        receiptIdsBySaleId={receiptIdsBySaleId}
      />
    </div>
  );
}
