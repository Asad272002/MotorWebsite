...
function buildRows(
  sales: readonly BikeSale[],
  partSales: readonly PartSale[],
  receiptIdsBySaleId: Readonly<Record<string, string>>,
): RegisterRow[] {
  const bikeRows = sales.map((sale) => {
    ...
    return {
      ...
      receiptHref: sale.receipt_generated && receiptIdsBySaleId[sale.id] ? `/admin/receipts/${receiptIdsBySaleId[sale.id]}` : undefined,
...
export function SalesRegisterClient({
  sales,
  partSales,
  receiptIdsBySaleId,
}: Readonly<{
  sales: BikeSale[];
  partSales: PartSale[];
  receiptIdsBySaleId: Record<string, string>;
}>) {
  const [query, setQuery] = useState("");
  const [type, setType] = useState<"all" | "bike" | "part">("all");
  const [status, setStatus] = useState("all");
  const rows = useMemo(() => buildRows(sales, partSales, receiptIdsBySaleId), [partSales, receiptIdsBySaleId, sales]);
