...
export default async function ReceiptPrintPage(props: { params: Promise<{ id: string }> }) {
  ...
  const grossAmount = Number(s.unit_price ?? 0) * qty || Number(s.total_amount ?? 0) + discountAmount;
              ...
              <table className="w-full border-collapse text-[12px]">
                ...
                <tfoot className="bg-[#FAFAFA] text-[12px]">
                  <tr className="border-t-2 border-[#C62828]/40">
                    <td colSpan={5} className="px-2.5 py-1.5 text-right font-semibold text-[#6B7280]">Original price</td>
                    <td className="px-2.5 py-1.5 text-right font-mono font-semibold">{grossAmount.toLocaleString("en-PK")}</td>
                  </tr>
                  {discountAmount > 0 ? (
                    <tr>
                      <td colSpan={5} className="px-2.5 py-1.5 text-right font-semibold text-[#15803D]">Discount</td>
                      <td className="px-2.5 py-1.5 text-right font-mono font-bold text-[#15803D]">- {discountAmount.toLocaleString("en-PK")}</td>
                    </tr>
                  ) : null}
                  <tr>
                    <td colSpan={5} className="px-2.5 py-1.5 text-right font-semibold text-[#111111]">Sale total</td>
                    <td className="px-2.5 py-1.5 text-right font-mono font-bold text-[#111111]">{(Number(s.total_amount) || 0).toLocaleString("en-PK")}</td>
                  </tr>
                  <tr>
