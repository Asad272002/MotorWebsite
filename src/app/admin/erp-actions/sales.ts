...
export async function initiateSale(_prev: AdminActionState, formData: FormData): Promise<AdminActionState> {
  ...
  if (discount >= grossTotal && discount > 0) {
    return {
      status: "error",
      message: "Discount must be less than the original sale price.",
      errors: { discountAmount: ["Enter a discount lower than the original sale price."] },
    };
  }
  ...
  if (totalPaidSubmitted > Number(total ?? 0)) {
    return {
      status: "error",
      message: `Payment exceeds the discounted sale total by PKR ${(totalPaidSubmitted - Number(total)).toLocaleString("en-PK")}.`,
      errors: { paymentsJson: ["Payment total must exactly match the final sale total after discount."] },
    };
  }
