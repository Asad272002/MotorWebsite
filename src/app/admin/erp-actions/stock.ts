...
import { canonicalizeBikeColor } from "@/lib/admin/standard-colors";
...
async function resolveStockBrand(
  sb: ReturnType<typeof serviceRoleClient>,
  input: { brandId?: string; newBrandName?: string | null },
): Promise<{ brand: { id: string; name: string; slug?: string | null; is_active?: boolean | null }; created: boolean } | Error> {
  if (input.brandId) {
    const brandRes = await sb.from("brands").select("id, name, slug, is_active").eq("id", input.brandId).maybeSingle();
    if (brandRes.error || !brandRes.data) return new Error("Select a valid existing brand.");
    const brand = brandRes.data as { id: string; name: string; slug?: string | null; is_active?: boolean | null };
    if (brand.is_active === false) return new Error("This brand is inactive. Activate the brand before adding stock.");
    return { brand, created: false };
  }

  const newBrandName = String(input.newBrandName ?? "").trim();
  if (!newBrandName) {
    return new Error("Select an existing brand or enter a new brand name.");
  }

  const slug = slugifyStockName(newBrandName);
  const [bySlug, byName] = await Promise.all([
    sb.from("brands").select("id, name, slug, is_active").eq("slug", slug).maybeSingle(),
    sb.from("brands").select("id, name, slug, is_active").ilike("name", newBrandName).maybeSingle(),
  ]);
  if (bySlug.error) return bySlug.error;
  if (byName.error) return byName.error;

  const existing = (bySlug.data ?? byName.data) as { id: string; name: string; slug?: string | null; is_active?: boolean | null } | null;
  if (existing) {
    if (existing.is_active === false) {
      return new Error(`Brand "${existing.name}" already exists but is inactive. Activate it before using it here.`);
    }
    return { brand: existing, created: false };
  }

  const orderRes = await sb.from("brands").select("display_order").order("display_order", { ascending: false }).limit(1);
  if (orderRes.error) return orderRes.error;
  const displayOrder = Number(orderRes.data?.[0]?.display_order ?? 0) + 1;

  const insert: Database["public"]["Tables"]["brands"]["Insert"] = {
    name: newBrandName,
    slug,
    short_description: `${newBrandName} showroom stock brand.`,
    full_description: `${newBrandName} was created from ERP stock entry. Complete public website content later if this brand should be published online.`,
    is_active: true,
    display_order: displayOrder,
  };
  const createdRes = await sb.from("brands").insert(insert).select("id, name, slug, is_active").maybeSingle();
  if (createdRes.error || !createdRes.data) return createdRes.error ?? new Error("Brand was not created.");
  return { brand: createdRes.data as { id: string; name: string; slug?: string | null; is_active?: boolean | null }, created: true };
}
...
export async function createSimpleBikeStock(_prev: AdminActionState, formData: FormData): Promise<AdminActionState> {
  ...
  const brandResult = await resolveStockBrand(sb, { brandId: parsed.data.brandId, newBrandName: parsed.data.newBrandName });
  if (brandResult instanceof Error) return { status: "error", message: brandResult.message };
  const { brand, created: brandCreated } = brandResult;
  ...
  const standardizedColor = canonicalizeBikeColor(parsed.data.colorName, parsed.data.colorHex);
  const variantInsert: Database["public"]["Tables"]["motorcycle_variants"]["Insert"] = {
    motorcycle_id: motorcycleId,
    cc: parsed.data.cc,
    color_name: standardizedColor.colorName,
    color_hex: standardizedColor.colorHex,
  ...
  try {
    await writeActivity({
      ...
      summary: `${actor.profile.full_name || actor.userId.slice(0, 8)} added bike stock item ${brand.name} ${modelName} ${parsed.data.cc}cc ${standardizedColor.colorName} with ${qty} unit(s).`,
  ...
  revalidateERP();
  return { status: "success", message: `${brand.name} ${modelName} added to stock with ${qty} unit(s).${brandCreated ? ` Brand "${brand.name}" was created.` : ""}` };
}
...
export async function createBikeStockVariant(_prev: AdminActionState, formData: FormData): Promise<AdminActionState> {
  ...
  const motorcycleRes = await sb
    .from("motorcycles")
    .select("id, name, brand_id, brand:brands(id, name, is_active), variants:motorcycle_variants(id, is_active, is_default, cc, price)")
  ...
  const templateVariant = ((motorcycleRes.data as unknown as {
    variants?: Array<{ id: string; is_active?: boolean | null; is_default?: boolean | null; cc?: number | null; price?: number | null }> | null;
  }).variants ?? [])
    .filter((variant) => variant.is_active !== false)
    .sort((a, b) => Number(b.is_default) - Number(a.is_default) || Number(a.cc ?? 0) - Number(b.cc ?? 0))[0] ?? null;
  if (!templateVariant) {
    return { status: "error", message: "This bike model has no active variant to inherit CC and price from." };
  }

  const standardizedColor = canonicalizeBikeColor(parsed.data.colorName, parsed.data.colorHex);
  const colorName = standardizedColor.colorName;
  const templateCc = Number(templateVariant.cc ?? 0);
  const templatePrice = Number(templateVariant.price ?? 0);
  const duplicate = await activeVariantExists(sb, parsed.data.motorcycleId, templateCc, colorName);
  if (duplicate instanceof Error) return databaseAction("createBikeStockVariant duplicate", duplicate);
  if (duplicate) return { status: "error", message: `${motorcycle.brand?.name ?? "Bike"} ${motorcycle.name} already has an active ${templateCc}cc ${colorName} variant.` };
  ...
  try {
    await writeActivity({
      ...
      actorRole: actor.profile.role,
      action: "stock_applied",
      summary: `${actor.profile.full_name || actor.userId.slice(0, 8)} added ${templateCc}cc ${colorName} variant to ${motorcycle.brand?.name ?? "Bike"} ${motorcycle.name} with ${qty} unit(s).`,
      targetTable: "motorcycle_variants",
      targetId: variantId,
      metadata: {
  ...
  revalidateERP();
  return { status: "success", message: `${motorcycle.brand?.name ?? "Bike"} ${motorcycle.name} ${templateCc}cc ${colorName} added.` };
}
export async function updateSimpleBikeStock(_prev: AdminActionState, formData: FormData): Promise<AdminActionState> {
  ...
  const standardizedColor = canonicalizeBikeColor(parsed.data.colorName, parsed.data.colorHex);
  const colorName = standardizedColor.colorName;
  const colorHex = standardizedColor.colorHex;
