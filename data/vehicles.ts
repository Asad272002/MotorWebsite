...
import bikeListingsJson from '@/bikes_data/bikesprices_cc/bikelistings.json';

const bikeListings = bikeListingsJson as BikeListing[];
...
const variantsByNormalizedName = bikeListings.reduce((acc, row) => {
  const baseName =
    row.original_replica.toLowerCase() === 'replica'
      ? row.name_of_bike.trim()
      : getBaseName(row.name_of_bike);

  const key = normalizeBikeName(baseName);
  if (!acc[key]) acc[key] = [];
  acc[key].push({
    name: baseName,
    cc: row.cc,
  ...
  return acc;
}, {} as Record<string, BikePriceVariant[]>);

Object.keys(variantsByNormalizedName).forEach((key) => {
  const list = variantsByNormalizedName[key] ?? [];
  const byKey = new Map<string, BikePriceVariant>();
  list.forEach((v) => {
    const variantKey = (v.variant ?? '').toLowerCase().replace(/[^a-z0-9]/g, '');
    const dedupeKey = `${v.cc}-${variantKey}`;
    const existing = byKey.get(dedupeKey);
    if (!existing || v.pricePkr > existing.pricePkr) {
      byKey.set(dedupeKey, v);
    }
  });
  variantsByNormalizedName[key] = Array.from(byKey.values()).sort((a, b) => {
    if (a.cc !== b.cc) return a.cc - b.cc;
    return (a.variant ?? '').localeCompare(b.variant ?? '');
  });
});

export const formatPricePkr = (value: number) => {
  const formatted = new Intl.NumberFormat('en-PK').format(value);
  return `PKR ${formatted}`;
};

export const getBikeVariants = (bikeName: string): BikePriceVariant[] => {
  const key = normalizeBikeName(getBaseName(bikeName));
  return variantsByNormalizedName[key] ?? [];
};

export const getBikePriceForCc = (bikeName: string, cc: number) => {
  const variants = getBikeVariants(bikeName);
  const matches = variants.filter((v) => v.cc === cc);
  if (matches.length === 0) return null;
  return matches.reduce((best, cur) => (cur.pricePkr > best.pricePkr ? cur : best));
};

export const getBikePricesForCc = (bikeName: string, cc: number) => {
  const variants = getBikeVariants(bikeName);
  return variants.filter((v) => v.cc === cc);
};

export const getBikeTopVariant = (bikeName: string) => {
  const variants = getBikeVariants(bikeName);
  if (variants.length === 0) return null;
  const topCc = Math.max(...variants.map((v) => v.cc));
  const topVariants = variants.filter((v) => v.cc === topCc);
  return topVariants.reduce((best, cur) => (cur.pricePkr > best.pricePkr ? cur : best));
};
